...
-- Crop to centered rectangle
function M.CenterCropHW(sizeH, sizeW)
    return function(input)
        local w1 = math.ceil((input:size(3) - sizeW)/2)
        local h1 = math.ceil((input:size(2) - sizeH)/2)
        return image.crop(input, w1, h1, w1 + sizeW, h1 + sizeH) -- center patch
    end
end
...
function M.CropHW(sizeH, sizeW, padding)
       return function(input)
            return image.crop(input,0,0,sizeW,sizeH)
        end
end

-- Random crop form larger image with optional zero padding
function M.RandomCropHW(sizeH, sizeW, padding)
   ...
   return function(input)
      ...
      local out = image.crop(input, x1, y1, x1 + sizeW, y1 + sizeH)
      assert(out:size(2) == sizeH and out:size(3) == sizeW, 'wrong crop size')
      return out
   end
end
